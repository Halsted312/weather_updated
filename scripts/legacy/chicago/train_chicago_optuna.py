#!/usr/bin/env python3
"""Train Chicago ordinal CatBoost model with Optuna tuning.

Uses parallel dataset building, then runs Optuna hyperparameter optimization.

Usage:
    PYTHONPATH=. .venv/bin/python scripts/train_chicago_optuna.py
    PYTHONPATH=. .venv/bin/python scripts/train_chicago_optuna.py --trials 80
    PYTHONPATH=. .venv/bin/python scripts/train_chicago_optuna.py --use-cached  # Skip dataset building
"""
...
def process_chunk(
    ...
    try:
        ...
        config = DatasetConfig(
            ...
            include_market=True,  # ENABLED: 12 market features from dense candles
...
def build_dataset_parallel(
    ...
    with ProcessPoolExecutor(max_workers=n_workers) as executor:
        futures = {
            executor.submit(process_chunk, i, city, chunk_start, chunk_end): i
...
def main():
    parser = argparse.ArgumentParser(description='Train Chicago CatBoost with Optuna')
parser.add_argument('--trials', type=int, default=DEFAULT_TRIALS, help='Number of Optuna trials')
parser.add_argument('--workers', type=int, default=N_WORKERS, help='Parallel workers for dataset building')
parser.add_argument('--use-cached', action='store_true', help='Use cached parquet files if available')
parser.add_argument('--cv-splits', type=int, default=4, help='Cross-validation splits')
    args = parser.parse_args()

    logger.info("=" * 60)
    logger.info(f"Chicago Optuna Training ({args.trials} trials)")
    logger.info("=" * 60)
    ...
    trainer = OrdinalDeltaTrainer(
        ...
        include_market=True,  # Use market features from Kalshi candles
        verbose=True,
    )
