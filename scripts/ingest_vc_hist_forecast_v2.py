...
This script correctly fetches historical forecasts (what VC predicted on past dates)
using station coordinates instead of station IDs. Station IDs (stn:KMDW) return
observation data, while lat/lon queries return actual forecast data.

Key differences from v1:
- Uses forecastBasisDay parameter (cleaner than forecastBasisDate)
- Uses lat/lon queries (not stn: queries) for forecast data
- Validates source field is 'fcst' (not 'obs') before storing
- Parallel processing by month

Usage:
    # Test with Chicago, one month
    python scripts/ingest_vc_hist_forecast_v2.py --city chicago --start-date 2024-07-01 --end-date 2024-07-31

    # Full backfill for Chicago
    python scripts/ingest_vc_hist_forecast_v2.py --city chicago --start-date 2023-01-01

    # Parallel by month
    python scripts/ingest_vc_hist_forecast_v2.py --city chicago --start-date 2023-01-01 --parallel
"""
...
def fetch_historical_forecast(
    ...
    Returns:
        API response dict with days (and optionally hours) data
    """
...
def process_month(
    ...
    Returns:
        Tuple of (daily_count, hourly_count, error_count)
    """
    city_cfg = get_city(city_id)
    latlon = city_cfg.vc_latlon_query
    ...
    logger.info(f"Processing {city_id} {year}-{month:02d}: {start_date} to {end_date}")
    ...
    with get_db_session() as db_session:
        # Get vc_location_id for this city (latlon type)
        # We'll store under the 'city' location type since lat/lon is city-aggregate-like
        ...
        if not location:
            logger.error(f"No VcLocation found for {city_cfg.city_code}/city")
            return 0, 0, 1
    ...
    logger.info(f"Completed {city_id} {year}-{month:02d}: {len(daily_records)} daily, {len(hourly_records)} hourly")
    return len(daily_records), len(hourly_records), error_count


def main():
    parser = argparse.ArgumentParser(description="Ingest VC historical forecasts using lat/lon")
    ...
    logger.info(f"Lat/Lon: {city_cfg.vc_latlon_query}")
    ...
    if args.parallel and len(months) > 1:
        with ThreadPoolExecutor(max_workers=args.workers) as executor:
            ...
            for future in as_completed(futures):
                ...
                try:
                    daily, hourly, errors = future.result()
                    total_daily += daily
                    total_hourly += hourly
                    total_errors += errors
        ...
        for year, month in months:
            daily, hourly, errors = process_month(
                args.city,
                year,
                month,
                lead_days_list,
    ...
    logger.info(f"COMPLETE: {total_daily} daily, {total_hourly} hourly, {total_errors} errors")
